...
// 
//时间堆
// 不管是单个链表还是时间轮散列表形式的定时器，都是固定频率
// 类似心跳，定时依次检测到期的定时器，调用处理函数
// 另一种思路是
// 将所有定时器中超时时间最小的一个定时器的超时值作为间隔
// 一旦tick函数调用，则至少最小的定时器到期
// 处理该定时器后，再从剩余定时器中找到最小的一个，设置为下一次超时间隔，如此反复
// 
// 最小堆是每个节点值都小于等于其子节点值的完全二叉树，很适合这种定时方案
// 数基本操作为插入和删除，最小堆中
// 插入操作，可以先在树下一个位置创建一个空节点，如果插入值后不破坏堆续则插入完成
//  否则，上虑操作，即交换空节点和父节点，循环执行直到值可以填入空节点而不破坏堆续
// 删除操作，删除的是根节点（最小元素），可以在根节点处创建空节点，将最后一个元素的节点值尝试填入空节点，不破坏堆续则填入并删除
//  否则，下虑操作，交换空节点和两个儿子节点中的较小者，再次查看最后一个元素节点值是否能填入空节点，如此反复
// 
// 另外，完全二叉树可以用数组表示，第i个位置的元素，左儿子位置为 2i+1,右儿子是 2i+2，父节点则是 (i-1)/2 上（i>0）
// 比链表更节约时间
// 包含N个元素的数组，初始化为最小堆，可对数组第(N-1)/2~0个元素执行下虑，可确保数组构成最小堆
// 因为对于N个元素的完全二叉树，有(N-1)/2个非叶子节点，即第0~(N-1)/2个节点，确保这些节点构成的子树都成堆，整个树就是堆
// 
// 例：最小堆实现时间堆定时器
// linux_src/time_heap_timer.hpp
//
...
// 
//通常需要考虑
// 统一事件源 将几种事件统一处理会让代码逻辑简单，避免一些潜在错误（I/O复用）
// 可移植性 不同操作系统的不同I/O复用方式
// 并发支持 多线程（进程）环境下，考虑客户连接、信号、定时器的协调
// 
//常用框架
// ACE、ASIO、Libevent
// 
//框架概述
// 库函数封装较底层的系统调用，提供便利接口
// 实现原理基本一致，要么是Reactor，要么是Proactor，或者结合
// Reactor模式一般包含几个组件
//  句柄 要处理的对象，即I/O事件、信号、定时事件，统称事件源，内核检测到事件时，通过句柄通知这个事件，linux下句柄即fd
//  事件多路分发器 事件是随机异步到达的，程序需要循环等待并处理，事件循环中，等待事件一般用I/O复用技术，将各系统支持的I/O复用封装统一接口
//  事件处理器 执行事件对应逻辑，通常包含一个或多个回调函数
//  Reactor 框架核心，执行事件循环（等待事件、一次处理就绪事件对应的事件处理器），调用多路分发器注册（删除）事件
// 
//Libevent
// 相对轻量的开源高性能跨平台I/O框架库
// 统一事件源
...
// 
// 例：Libevent最基本应用
// linux_src/libevent/hello_libevent.cpp
//
