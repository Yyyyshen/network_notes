...
//
//主机字节序和网络字节序
// 网络通信在本质是不同机器数据交换，数据以字节流方式传输，字节流排列方式分为主机字节序和网络字节序
// 
// 主机字节序根据不同的CPU、操作系统，存储字节顺序分为
//	LE小端，高位存储在高地址，低位存储在低地址，符合人思维，intel x86系列系统用小端
//	BE大端，存储顺序为高低低高，看起来可能更直观，JAVA和Mac一般是大端
// 例如，双字值 0x10 20 30 40，在内存----->方向中
// 小端：40 30 20 10
// 大端：10 20 30 40
// 
// 网络字节序
// TCP/IP中统一规定好的数据表示格式，与CPU和操作系统无关，保证数据在不同主机间传输中正确解释
// 采用的是大端
// 所以，一般需要将传输整型值转为网络字节序，即端口或地址，在设置时做处理
// 若本机字节就是大端，则什么都不做，否则将字节序互换
// 函数
